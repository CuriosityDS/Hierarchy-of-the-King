...
namespace Hierarchy_of_the_King
{
    public class Rectangle
    {
        ...
        {
            RectangleConvex temp = (RectangleConvex)tochka;
            if (this.S == temp.S)
                return true;
            else
                return false;
    ...
    public class RectangleConvex : Rectangle
    {
        Point[] points = new Point[4];
        Double ABC, BCD, CDA, DAB;
        double AB, BC, CD, DA, AC, BD;
        double S, P;
        public RectangleConvex() : base() { SearchAngles(); SearchSides(); SearchSquad(); }
        public RectangleConvex(Point p1, Point p2, Point p3, Point p4) : base(p1, p2, p3, p4) { SearchAngles(); SearchSides(); SearchSquad(); }
        public override string ToString()
        {
            return $"Выпуклый четырёхугольник: {base.ToString()}";
        }
    }
}
